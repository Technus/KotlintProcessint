...
class Text(
    position: Position = Position(0f, 0f),
    gridSize: Size = Size(32f, 32f),
    cellSize: Size = glyphSize * Constants.cellSize
) : Grid<Glyph>(position, gridSize, cellSize) {
  ...
  <defs>
    <font id="Quaghn${string}" horiz-adv-x="${(Constants.cellSize.w * glyphSize.w) / perEm}" >
      <font-face font-family="Quaghn"
          ...
          alphabetic="0" />
${
        cells.values.stream().filter { cell -> cell.char != '\u0000' }.map { cell -> cell.toGlyphSVG(perEm) }
            .filter { str -> str.isNotEmpty() }.toArray().joinToString(separator = "\n")
    }
    </font>
  </defs>
</svg>""".trimIndent()
    ...
    fun fromStringRepresentation(string: String) {
        ...
        lines.forEach { line ->
            if (!line.isBlank()) {
                ...
                cells[Position(positionPieces[0].toFloat(), positionPieces[1].toFloat())]!!.fromStringRepresentation(
                    entryPieces[0]
                )
            }
        }
