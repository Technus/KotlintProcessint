...
class Glyph(
    position: Position,
    gridSize: Size = Constants.glyphSize,
    cellSize: Size = Constants.cellSize,
    var char: Char = '\u0000'
) : Grid<Cell>(position, gridSize, cellSize) {
    init {
    ...
    fun toGlyphSVG(perEm: Float): String = if (char.isLetterOrDigit())
        ...
        "<glyph unicode=\"&#x${
            char.toInt().toString(16).padStart(4, '0')
        };\" horiz-adv-x=\"${size.w / perEm}\" d=\"${toSVG(true, perEm)}\" />"
    ...
    private fun fixShit(validCheck: Boolean) {
        var valid = true
