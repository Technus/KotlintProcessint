...
open class Area(val position: Position, val size: Size) {
    ...
    fun envelops(point: Position?): Boolean =
        point != null && point.x > position.x && point.x < endPosition.x && point.y > position.y && point.y < endPosition.y
    ...
    fun innerPoint(point: MouseEvent): Position =
        Position((point.x - position.x) / size.w, (point.y - position.y) / size.h)
}